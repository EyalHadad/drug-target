...
def download_targets_data(is_train):
    ...
    dw.connect()
    dw.get_table(schema="uniport",table="gene",dst_file_path=os.path.join(dir_path,"protein_gene.csv"),headers=['protein','gene'])
    dw.get_table(schema="uniport",table="keywords",dst_file_path=os.path.join(dir_path,"protein_keywords.csv"),headers=['protein','keywords'])
    dw.get_table(schema="uniport",table="taxonomic_lineage",dst_file_path=os.path.join(dir_path,"protein_taxon.csv"),headers=['protein','taxon'])
    dw.disconnect()
    if is_train=="train":
