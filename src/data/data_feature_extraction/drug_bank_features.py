import random
import os
from src.data.data_feature_extraction.table_names import *
from src.data.data_feature_extraction.data_migration_handler import *
from src.data.data_feature_extraction.get_drug_bank_features import GetDrugBankFeaturesFromDB
...
def get_drug_modalities_data(is_train, version):
    random.seed(30)
    print("-----Get drugbank modalities data version", version, "-----")
    x_unlabeled, modalities_df = get_csvs(version)
    print("-----modalities data was created-----")
    print("-----Start calculate clusters-----")
    calculate_clusters(x_unlabeled, modalities_df, os.path.join(EXTERNAL_PATH, is_train))
